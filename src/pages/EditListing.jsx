...
const EditListing = () => {
  ...
  // useEfferct ot check login user
  useEffect(() => {
  ...
  const onSubmit = async (e) => {
    ...
    if (geoLocationEnable) {
      const response = await fetch(
        `https://maps.gooleapis.com/maps/api/geocode/json?address=${address}&key=AIzaSyDb12RZOam9ysr5p45Zd1h0Z8HUsnksANE`
      );
  ...
  return (
    <Layout>
      <div className="container d-flex flex-coloumn align-items-center justify-content-center mb-4">
        <form className="w-50 bg-light p-4" onSubmit={onSubmit}>
          <h3 className="mt-3 w-100 bg-dark text-light p-2 text-center">
            Update Listing &nbsp;
            <AiOutlineFileAdd />
          </h3>
          ...
          {offer && (
            <div className="mb-3 mt-4">
              <label htmlFor="dicountedPrice" className="form-label">
                Discounted Price :
              </label>
              <input
                type="number"
